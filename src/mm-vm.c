...
int is_in_freerg_list(struct pcb_t *caller, int vmaid, struct vm_rg_struct *currg)
{
  ...
  while (rgit != NULL)
  {
    ...
    rgit = rgit->rg_next; // Go to the next region
  }
  return 1;
}
...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  pthread_mutex_lock(&vmlock); // Acquire the mutex lock to ensure thread-safe access to the memory management structures
  ...
  if (get_free_vmrg_area(caller, vmaid, size, &rgnode) == 0)
  {
    ...
    int inc_sz = PAGING_PAGE_ALIGNSZ(rgnode.rg_end - rgnode.rg_start);
    int inc_numpages = inc_sz / PAGING_PAGESZ;

    if (vm_map_ram(caller, rgnode.rg_start, rgnode.rg_end, rgnode.rg_start, inc_numpages, new_rg) < 0)
    {
  ...
  *alloc_addr = old_sbrk; // Set the allocation address to the old break value
  ...
  if (old_sbrk + size < remain_rg->sbrk)
  {
    ...
    free_region->rg_start = old_sbrk + size;
    free_region->rg_end = remain_rg->sbrk;
...
#ifdef CPU_TLB
    /* Update TLB: Invalidate old entry and update with new frame number */
    tlb_cache_write(caller->tlb, caller->pid, vicpgn, (BYTE)swpfpn);
    tlb_cache_write(caller->tlb, caller->pid, pgn, (BYTE)vicfpn);
#endif
  }
...
int find_victim_page(struct mm_struct *mm, int *retpgn)
{
  ...
  struct pgn_t *last_pgn = pg->pg_next;

  *retpgn = last_pgn->pgn; // Store the page number of the last page

  pg->pg_next = NULL; // Remove the last page from the list by setting the next pointer of the second-to-last page to NULL

  free(pg); // Free the removed page
