...
#ifdef CPU_TLB
    tlb_cache_write(caller->tlb, caller->pid, pgn + pgit, (BYTE)fpit->fpn); // Update TLB
#endif

#ifdef IODUMP
    printf("\t\tPID: %d --- Mapped page %d to frame %d\n", caller->pid, pgn + pgit, fpit->fpn);
#endif
  ...
  for (pgit = 0; pgit < req_pgnum; pgit++)
  {
    ...
    {                          // ERROR CODE of obtaining somes but not enough frames
      ...
      if (find_victim_page(caller->mm, &victim_page) == -1 || MEMPHY_get_freefp(caller->active_mswp, &swpfpn) == -1)
      {
        ...
        if (*frm_lst == NULL)
          return -1; // No frames allocated at all, return -1.
